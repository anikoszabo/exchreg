...
run_analysis <- function(dat, cs_predict=5){
  ...
  fit <- bind_rows(fit_bb, fit_bin, fit_spglm) %>%
    mutate(Type = "PDF")
...
comb <- bind_rows(`Both correct` = sim1, 
  ...
  mutate(Generator= factor(Generator, levels = c("Both correct",
                                                 ...
                                                 "SPGLM correct",
                                                 "Neither correct")),
         Model = factor(Model, levels = c("BB","SPGLM","GEE")))
...
ggplot(comb_beta, aes(x=Term, y=Est-Correct, fill=Model, outlier.fill=Model)) +
  facet_wrap(~Generator) +
  geom_hline(aes(yintercept=0)) +
  geom_boxplot(position=position_dodge(width=0.4), width=0.3, 
               outlier.size =1, outlier.alpha=0.5) +
  scale_y_continuous("Bias = Estimate - correct value") +
